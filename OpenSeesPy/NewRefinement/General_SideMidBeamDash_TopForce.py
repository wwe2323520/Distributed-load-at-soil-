...
# ----- Left Side: 145~165 -----------------
    ...
    if i < ny: #146,148...,164
        ...
        fix((LSideBeamNode_Start+1)+2*i,0,0,1)
...
# ----- Right Side: 166~186 / 167~185 -----------------
    ...
    if i < ny: #146,148...,164
        ...
        fix((RSideBeamNode_Start+1)+2*i,0,0,1)
...
# ----- Left Side Beam:107 ~ 126 -----------------
    element('elasticBeamColumn', LSideBeam_Start+2*j, LSideBeamNode_Start+2*j, (LSideBeamNode_Start+1)+2*j, A,E1,Iz, 1, '-release', 1)
    element('elasticBeamColumn', (LSideBeam_Start+1)+2*j, (LSideBeamNode_Start+1)+2*j, (LSideBeamNode_Start+2)+2*j, A,E1,Iz, 1, '-release', 2)
# ----- Right Side Beam:127 ~ 146 -----------------
    element('elasticBeamColumn', RSideBeam_Start+2*j, RSideBeamNode_Start+2*j, (RSideBeamNode_Start+1)+2*j, A,E1,Iz, 1, '-release', 1)
    element('elasticBeamColumn', (RSideBeam_Start+1)+2*j, (RSideBeamNode_Start+1)+2*j, (RSideBeamNode_Start+2)+2*j, A,E1,Iz, 1, '-release', 2)
...
LSideTDash_Start =  LSideNDash_Start + 2*ny +1
RSideNDash_Start =  LSideTDash_Start + 2*ny
RSideTDash_Start =  RSideNDash_Start + 2*ny
# print(LSideNDash_Start,LSideTDash_Start)
# print(RSideNDash_Start,RSideTDash_Start)
for l in range(ny):
# ========= Left Side =============
# --------- Normal dashpot (node 187,188~ 205,206)-> for S wave------------
    node(LSideNDash_Start+2*l, 0.0, (0.5*yMesh) + yMesh*l)
    node((LSideNDash_Start+1)+2*l, 0.0, (0.5*yMesh) + yMesh*l)
# ---------- dashpot dir: Vs -> x dir ---------------------     
    fix(LSideNDash_Start +2*l, 0, 1, 1)      # x dir dashpot　
    fix((LSideNDash_Start+1)+2*l, 1, 1, 1)      # fixed end to let soil fix

# --------- Traction dashpot (node 208,209~ 226,227)-> for P wave------------
    node(LSideTDash_Start+2*l, 0.0, (0.5*yMesh) +yMesh*l)
...
# ========= Right Side =============
# --------- Normal dashpot (node 228,229~ 246,247)-> for S wave------------
    node(RSideNDash_Start+2*l, soilwidth, (0.5*yMesh) + yMesh*l)
...
# ========= Left Side =============
# --------------Normal dashpot: for S wave------------------
    equalDOF((LSideBeamNode_Start+1)+2*l, LSideNDash_Start+2*l, 1)  # x dir
# # --------------Traction dashpot: for P wave------------------
    equalDOF((LSideBeamNode_Start+1)+2*l, LSideTDash_Start+2*l, 2)  # y dir

# ========= Right Side =============
# --------------Normal dashpot: for S wave------------------
    equalDOF((RSideBeamNode_Start+1)+2*l, RSideNDash_Start+2*l, 1)  # x dir
# --------------Traction dashpot: for P wave------------------
...
S_Cmp = 1.0*rho*Vp*sizeX1    # side Normal dashpot for S wave: Netwon
S_Cms = 1.0*rho*Vs*sizeX1    # side Traction dashpot for P wave: Netwon 1.25
...
# print(RsideNEle_Start,RsideTEle_End)
RsideTEle_Start = RsideNEle_End + 1
...
#  ----------- Left side Normal: S wave ----------
element('zeroLength',LsideNEle_Start, (LSideNDash_Start+1), LSideNDash_Start, '-mat',4004,'-dir',xdir)  #Lower Left node  -> Smp
element('zeroLength',LsideNEle_End, (LSideNDash_Start+1)+2*(ny-1), LSideNDash_Start+2*(ny-1), '-mat',4004,'-dir',xdir)  # Upper Left node:-> Smp
#  ----------- Left side Traction: P wave ----------
element('zeroLength',LsideTEle_Start, (LSideTDash_Start+1), LSideTDash_Start, '-mat',4005,'-dir',ydir)  #Lower Left node  -> Sms
element('zeroLength',LsideTEle_End, (LSideTDash_Start+1)+2*(ny-1), LSideTDash_Start+2*(ny-1), '-mat',4005,'-dir',ydir) # Upper Left node: -> Sms

#  ----------- Right side Normal: S wave ----------
element('zeroLength',RsideNEle_Start, (RSideNDash_Start+1), RSideNDash_Start, '-mat',4004,'-dir',xdir)  #Lower Right node  -> Sms -> Smp
element('zeroLength',RsideNEle_End, (RSideNDash_Start+1)+2*(ny-1), RSideNDash_Start+2*(ny-1), '-mat',4004,'-dir',xdir)  # Upper Right node:-> Smp
#  ----------- Right side Traction: P wave ----------
element('zeroLength',RsideTEle_Start, (RSideTDash_Start+1), RSideTDash_Start, '-mat',4005,'-dir',ydir)  #Lower Right node -> Sms
...
# print(RsideTEle_End, (RSideTDash_Start+1)+2*(ny-1), RSideTDash_Start+2*(ny-1))
...
for o in range(2*ny):
    element('twoNodeLink',LtwoNodeBeam+o, LSideBeamNode_Start+o,  (LSideBeamNode_Start+1)+o,'-mat',5000,'-dir',1,2)
    element('twoNodeLink',RtwoNodeBeam+o, RSideBeamNode_Start+o, (RSideBeamNode_Start+1)+o,'-mat',5000,'-dir',1,2)

# ----- Another Test about Link element with Dashpot --------------------------
# LtwoTNodeBeam = LtwoNodeBeam + 2*ny #207
# RtwoNodeBeam = LtwoTNodeBeam + 2*ny
# RtwoTNodeBeam = RtwoNodeBeam + 2*ny
# # print(RtwoTNodeBeam)
# for o in range(ny):
# # ========== Left Node Normal Dashpot ===================
#     element('twoNodeLink',LtwoNodeBeam+2*o, LSideBeamNode_Start+2*o,  LSideNDash_Start+2*o,'-mat',5000,'-dir',1)
#     element('twoNodeLink',(LtwoNodeBeam+1)+2*o, LSideNDash_Start+2*o,  (LSideBeamNode_Start+2)+2*o,'-mat',5000,'-dir',1)
# # ========== Left Node traction Dashpot ===================
#     element('twoNodeLink',LtwoTNodeBeam+2*o, LSideBeamNode_Start+2*o,  LSideTDash_Start+2*o,'-mat',5000,'-dir',2)
#     element('twoNodeLink',(LtwoTNodeBeam+1)+2*o, LSideTDash_Start+2*o,  (LSideBeamNode_Start+2)+2*o,'-mat',5000,'-dir',2)

# # ========== Right Node Normal Dashpot ===================(227~246)
#     element('twoNodeLink',RtwoNodeBeam+2*o, RSideBeamNode_Start+2*o,  RSideNDash_Start+2*o,'-mat',5000,'-dir',1)
#     element('twoNodeLink',(RtwoNodeBeam+1)+2*o, RSideNDash_Start+2*o,  (RSideBeamNode_Start+2)+2*o,'-mat',5000,'-dir',1)

# # ========== Right Node traction Dashpot ===================(247~266)
#     element('twoNodeLink',RtwoTNodeBeam+2*o, RSideBeamNode_Start+2*o,  RSideTDash_Start+2*o,'-mat',5000,'-dir',2)
#     element('twoNodeLink',(RtwoTNodeBeam+1)+2*o, RSideTDash_Start+2*o,  (RSideBeamNode_Start+2)+2*o,'-mat',5000,'-dir',2)
    # print((RtwoTNodeBeam+1)+2*o, RSideTDash_Start+2*o,  (RSideBeamNode_Start+2)+2*o)
# ============================== S wave ======================================
...
# ---------- x direction : Sideforce ---------------------
# ---------- Distributed at Left Side Beam ----------------------
    eleLoad('-ele',LSideBeam_Start+g, '-type', '-beamUniform',-0.5,0)  # for local axes Wy -
# ---------- Distributed at Right Side Beam ----------------------
