...
for akz in a_cofficient:
    for bkz in b_cofficient:        
        ...
        # ================= Build Boundary File =====================
        path1 = f'RayleighDashpot\{Boundary}\Ca{akz}_Cb{bkz}\Velocity'
        path2 = f'RayleighDashpot\{Boundary}\Ca{akz}_Cb{bkz}\Stress'
        path3 = f'RayleighDashpot\{Boundary}\Ca{akz}_Cb{bkz}\SurfaceVelocity'
...
# ------------ For Tie Boundary -----------------------
        ...
        LTDash_Raly = LNDash_Raly + 2
        # ------- Normal Dashpot ------------
        node(LNDash_Raly, 0.0, soilLength)
        node(LNDash_Raly+1, 0.0, soilLength)
        # ---------- dashpot dir: Vs -> x dir ---------------------     
        fix(LNDash_Raly, 0, 1, 1)      # x dir dashpot　
        fix(LNDash_Raly+1, 1, 1, 1)      # fixed end to let soil fix

        # ------- Traction Dashpot ------------
        node(LTDash_Raly, 0.0, soilLength)
        node(LTDash_Raly+1 , 0.0, soilLength)
        # ---------- dashpot dir: Vs -> Y dir ---------------------     
        fix(LTDash_Raly, 1, 0, 1)      # x dir dashpot　
        fix(LTDash_Raly+1, 1, 1, 1)      # fixed end to let soil fix

        # ---------- Right Side Rayleigh Dashpot ---------------------
        RNDash_Raly = LTDash_Raly + 2
        RTDash_Raly = RNDash_Raly + 2
        # ------- Normal Dashpot ------------
        node(RNDash_Raly, soilwidth, soilLength)
        node(RNDash_Raly+1, soilwidth, soilLength)
        # ---------- dashpot dir: Vs -> x dir ---------------------     
        fix(RNDash_Raly, 0, 1, 1)      # x dir dashpot　
        fix(RNDash_Raly+1, 1, 1, 1)      # fixed end to let soil fix

        # ------- Traction Dashpot ------------
        node(RTDash_Raly, soilwidth, soilLength)
        node(RTDash_Raly+1 , soilwidth, soilLength)
        # ---------- dashpot dir: Vs -> Y dir ---------------------     
        fix(RTDash_Raly, 1, 0, 1)      # x dir dashpot　
        fix(RTDash_Raly+1, 1, 1, 1)      # fixed end to let soil fix

        # ------ connect dashpot with Soil side layer :Vs with x dir / Vp with y-dir --------------
        # ========= Left Side =============
        # --------------Normal dashpot: for S wave------------------
        equalDOF(1+(nx+1)*ny, LNDash_Raly, 1)  # x dir
        # --------------Traction dashpot: for P wave------------------
        equalDOF(1+(nx+1)*ny, LTDash_Raly, 2)  # y dir

        # ========= Right Side =============
        # --------------Normal dashpot: for S wave------------------
        equalDOF((nx+1)+(nx+1)*ny, RNDash_Raly, 1)  # x dir
        # --------------Traction dashpot: for P wave------------------
        equalDOF((nx+1)+(nx+1)*ny, RTDash_Raly, 2)  # y dir
        ...
        Ray_Smp = 0.5*akz*rho*Vp*Dwidth         # upper Left and Right corner node dashpot :N (newton) 0.5
        Ray_Sms = 0.5*bkz*rho*Vs*DMeshSize      # upper Left and Right corner node dashpot :N (newton) 0.
...
# --------- For Tie Boundary -------------------------
        ...
        element('zeroLength',LN_Rayele, LNDash_Raly+1, LNDash_Raly, '-mat',4010,'-dir',xdir)  # node 1: Left side
        # ------ Traction dashpot element: Vp with y dir
        element('zeroLength',LT_Rayele, LTDash_Raly+1, LTDash_Raly, '-mat',4011,'-dir',ydir)  # node 8: Right side

        RN_Rayele = LT_Rayele + 1
        RT_Rayele = RN_Rayele + 1
        # ------ Normal dashpot element: Vs with x dir
        element('zeroLength',RN_Rayele, RNDash_Raly+1, RNDash_Raly, '-mat',4010,'-dir',xdir)  # node 1: Left side
        # ------ Traction dashpot element: Vp with y dir
        ...
        # ------------- left column -------------
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{LowerE_Left}.out', '-time', '-ele',LowerE_Left, 'material ',1,'stresses')
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{CenterE_Left}.out', '-time', '-ele',CenterE_Left, 'material ',1,'stresses')
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{UpperE_Left}.out', '-time', '-ele',UpperE_Left, 'material ',1,'stresses')
        ...
        # ------------- Center column -------------
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{LowerE_Center}.out', '-time', '-ele',LowerE_Center, 'material ',1,'stresses')
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{CenterE_Center}.out', '-time', '-ele',CenterE_Center, 'material ',1,'stresses')
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{UpperE_Center}.out', '-time', '-ele',UpperE_Center, 'material ',1,'stresses')
        ...
        # ------------- Right column -------------
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{LowerE_Right}.out', '-time', '-ele',LowerE_Right, 'material ',1,'stresses')
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{CenterE_Right}.out', '-time', '-ele',CenterE_Right, 'material ',1,'stresses')
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{UpperE_Right}.out', '-time', '-ele',UpperE_Right, 'material ',1,'stresses')
        ...
        # ==== Left 1/4 node ======================================
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{UpperrE_LQuarter}.out', '-time', '-ele',UpperrE_LQuarter, 'material ',1,'stresses')
        # recorder('Node', '-file', f'Velocity/node{UpperrN_LQuarter}.out', '-time', '-node',UpperrN_LQuarter,'-dof',1,2,3,'vel')

        # ==== Right 1/4 node ======================================
        recorder('Element', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Stress/ele{UpperrE_RQuarter}.out', '-time', '-ele',UpperrE_RQuarter, 'material ',1,'stresses')
        ...
        # ---------------- Left Side ------------
            ...
            recorder('Node', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Velocity/D{Depth}_Lnode{LeftNode}.out', '-time', '-node',LeftNode,'-dof',1,2,3,'vel')
            recorder('Node', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Velocity/D{Depth}_Cnode{CenterNode}.out', '-time', '-node',CenterNode,'-dof',1,2,3,'vel')
            recorder('Node', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/Velocity/D{Depth}_Rnode{RightNode}.out', '-time', '-node',RightNode,'-dof',1,2,3,'vel')
        # --------------- Top X(m) Each Dw  -----------------------
        ...
        for width in range(nx+1):
            ...
            recorder('Node', '-file', f'RayleighDashpot/{Boundary}/Ca{akz}_Cb{bkz}/SurfaceVelocity/W{MeshWidth}node{topNode}.out', '-time', '-node',topNode,'-dof',1,2,3,'vel')
            # print(topNode)
