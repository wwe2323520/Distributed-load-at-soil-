...
Extend_Scale = 1# 100
# size = np.array([1e+1, 1e+2, 1e+3]) #1e-0, 1e-1, 1e-2, 1e-3, 1e-4
# # time_Scale = 1e-4

# Compare = f"Parameter_Compare/Compare_rho"
# Compare_For = f"rho"
# for Scale in size:
#     print(f"scale = {Scale}")
wipe()
...
for i in range(ny+1):
    ...
    fix(LeftNode+i, 1, 1)
    fix(RightNode+i, 1, 1)
    # print(RightNode+i, soilwidth+widthMesh, yMesh*i)
...
region(10000, '-eleRange ', BotEle, BotEle+ny-1,'-rayleigh',alphaM*1.0, betaK, betaKinit,betaKcomm) 

# ----------- Left Side Rayleigh Damping ----------------------------
alphaM_L = (2/Ghost_yMesh)*(rho/rho2)*(-Vp + 2*Vs)  # (2/Ghost_yMesh)*(rho/rho2)*(-Vp + 2*Vs) /(original force) (2/Ghost_yMesh)*(rho/rho2)*(2*Vp + Vs) 
betaK_L =  (2*Ghost_yMesh)*(1/E_G)*rho*(Vp-Vs) #   (2*Ghost_yMesh)*(1/E_G)*rho*(Vp-Vs) /(original force) (2*Ghost_yMesh)*(1/E_G)*rho*(-Vp-Vs)
print(f"alphaM_L = {alphaM_L}; betaK_L = {betaK_L}")
# ----------- Right Side Rayleigh Damping ----------------------------
alphaM_R = (2/Ghost_yMesh)*(rho/rho2)*(-Vp + 2*Vs)  #(2/Ghost_yMesh)*(rho/rho2)*(-Vp + 2*Vs) /(original force) (2/Ghost_yMesh)*(rho/rho2)*(-2*Vp - Vs) 
betaK_R =  (2*Ghost_yMesh)*(1/E_G)*rho*(Vp-Vs)     #(2*Ghost_yMesh)*(1/E_G)*rho*(Vp-Vs) /(original force) (2*Ghost_yMesh)*(1/E_G)*rho*(Vp+Vs)
print(f"alphaM_R = {alphaM_R}; betaK_R = {betaK_R}")

# --------------- Left / Right Rayleigh Damping C = alphaM * M + betaK * K ---------------------------
region(10001, '-eleRange ', LeftEle+1, LeftEle+ny-1,'-rayleigh', alphaM_L, betaK_L, betaKinit,betaKcomm) 
region(10002, '-eleRange ', RightEle+1, RightEle+ny-1,'-rayleigh',alphaM_R, betaK_R, betaKinit,betaKcomm) 

region(10001, '-eleRange ', LeftEle, LeftEle,'-rayleigh', alphaM_L, betaK_L*1.0, betaKinit,betaKcomm) 
region(10002, '-eleRange ', RightEle, RightEle,'-rayleigh',alphaM_R, betaK_R*1.0, betaKinit,betaKcomm) 
...
for j in range(ny+1):
    ...
    # print((nx+1)+(nx+1)*j,RsideNode+j,1,2)
