...
Extend_Scale = 1# 100
# size = np.array([1e-0, 1e-1, 1e-2, 1e-3, 1e-4]) #1e-0, 1e-1, 1e-2, 1e-3, 1e-4
# time_Scale = 1e-4

# Compare = f"ReactionForce/Compare_rho_1e-3E"
# Compare_For = f"rho"
# for Scale in size:
#     print(f"scale = {Scale}")
wipe()
...
soilLength = 0.125 #m 10*Extend_Scale
soilwidth = 10   #1.0/ 0.125

nx = 80 # int(soilwidth/(0.125*Extend_Scale))
ny = 1 # 80
e1 = 1
n1 = 1
...
# points = [1, 0.0,   soilLength, 
#         2, 0.0,  0.0, 
#         3, soilwidth, 0.0, 
#         4, soilwidth, soilLength]
# block2D(nx, ny, e1, n1,'quad', *eleArgs, *points)
for j in range(nx+1):
    node(1+j , 0.125*j, 0.0)
    node((nx+2)+j, 0.125*j, 0.125)

for k in range(nx):
    element('quad', e1+k, (nx+2)+k, n1+k, (n1+1)+k, (nx+3)+k,  1, 'PlaneStrain', 2000)
    # print(e1+k, (nx+2)+k, n1+k, (n1+1)+k, (nx+3)+k)
...
for i in range(nx+1):
    equalDOF((nx+1)+(i+1), (i+1),1,2)
    # print((nx+1)+(i+1), (i+1),1,2)
...
yMesh = soilwidth/nx # Y row MeshSize
dcell = yMesh / Vs 
dt = dcell/10.0  #6.25e-5
print(f"yMesh = {yMesh}; dcell = {dcell}")
# ======= Totla Analysis Time ============================
analysisTime = (soilwidth/Vs)*8
analystep = analysisTime/dt # int(800*(ny/10))
print(f"dt = {dt}; Analysis Total Time = {analysisTime} ;Analysis_step = {analystep}")
...
# # ================= Build Boundary File =====================H{Applt_D}/Ca{akz}_Cb{bkz}/Stress
# path1 = f'D:/shiang/opensees/20220330/OpenSeesPy/Velocity/Column_Ray/{Compare}/{Scale}{Compare_For}'
# path2 = f'D:/shiang/opensees/20220330/OpenSeesPy/Stress/Column_Ray/{Compare}/{Scale}{Compare_For}'
...
Ghost_yMesh = 1*0.125 
print(f"Ghost_yMesh = {Ghost_yMesh}")

# for i in range(nx+1):
#     node(BotNode+i, widthMesh*i, -Ghost_yMesh*Extend_Scale)
#     fix(BotNode+i, 1 ,1) # like LK Dashpot have the fix end to get reaction Rayleigh Force
#     # print(BotNode+i, widthMesh*i, -Ghost_yMesh*Extend_Scale)

for i in range(ny+1):
    node(BotNode+i, -widthMesh, Ghost_yMesh- i*Ghost_yMesh)
    fix(BotNode+i, 1 ,1) 
    # print(BotNode+i, -widthMesh, Ghost_yMesh- i*Ghost_yMesh)
...
nu2 = 0.0
Scale = 1e-0 # Control Size  (1/10000) 
# E_M = Scale* M*((1+nu2)*(1-2*nu2)/(1-nu2)) # young's Module Control by M (P wave Velocity) # (N/m^2)= (kg*m/s2)*(1/m2) = kg/(s2*m) 
E_G = 1e-4* 2*G*(1+nu2) # young's Module Control by G (S wave Velocity) # (N/m^2)= (kg*m/s2)*(1/m2) = kg/(s2*m) 
...
for k in range(ny):
    element('quad', BotEle+k, BotNode+k, (BotNode+1)+k, n1+k,  (nx+2)+k, 1, 'PlaneStrain', 2006)
    # print( BotEle+k, BotNode+k, (BotNode+1)+k, n1+k,  (nx+2)+k)

# # =============Apply Left and Right virtual Quad element to apply Rayleigh Dashpot =======================
# LeftNode =  endNode+ 1  #BotNode+ nx + 1
# RightNode = LeftNode + ny + 1

# LeftEle = endEle+1 # BotEle + nx
# RightEle = LeftEle + ny
...
#     node(LeftNode+i, -widthMesh, yMesh*i)
    ...
    # fix(LeftNode+i, 1, 1)
    # fix(RightNode+i, 1, 1)
    # print(LeftNode+i, -widthMesh, yMesh*i)
...
# M = printA('-file','M.out') #, '-ret' / Or use ops.printA('-file','M.out')

# # Stiffness
# integrator('GimmeMCK',0.0,0.0,1.0)
# analyze(1,0.0)
# K = printA('-file','K.out') # ,'-ret'
# # K = np.array(K)
# # K.shape = (N,N)
# # print(K)
...
# modalProperties('-print', '-file', 'ModalReport4GM.txt', '-unorm')
# =========== Bottom Beam element ===================
# --------- Bot Virtual quad -----------
# BeamNode_Start = BotNode + (nx+1) 
# BeamEle_Start = BotEle + nx

BeamNode_Start = BotNode + ny + 1  
BeamEle_Start = BotEle + ny 
...
for j in range(ny+1):
    node(BeamNode_Start+j, 0.0, 0.125-j*0.125) 
    mass(BeamNode_Start+j,1,1,1)
# -------- fix rotate dof ------------
    fix(BeamNode_Start+j,0,0,1)
...
element('elasticBeamColumn', BeamEle_Start+k, BeamNode_Start+1+k, BeamNode_Start+k, A,E1,Iz, 1, '-release', 3)

# =========== connect bot beam and soil element =========================
# for k in range(nx+1):
#     equalDOF(n1+k,BeamNode_Start+k,1,2)

equalDOF(nx+2,BeamNode_Start,1,2)
equalDOF(n1,BeamNode_Start+1,1,2)
...
# betaKcomm = 0.0
# ----------- Lumped Mass ----------------------------
alphaM = (2/Ghost_yMesh)*(rho/rho2)*(-Vp + 2*Vs) 
betaK = (2*Ghost_yMesh)*(1/E_G)*rho*(Vp-Vs)
betaKinit = 0.0
betaKcomm = 0.0
...
region(10000, '-eleRange ', BotEle, BotEle+ny-1,'-rayleigh',alphaM, betaK, betaKinit,betaKcomm) 

# # # ----------- Left Side Rayleigh Damping ----------------------------
# # alphaM_L = (2/Ghost_yMesh)*(rho/rho2)*(2*Vp + Vs) 
# # betaK_L = (2*Ghost_yMesh)*(1/E_G)*rho*(-Vp-Vs)
# # print(f"alphaM_L = {alphaM_L}; betaK_L = {betaK_L}")
# # # ----------- Right Side Rayleigh Damping ----------------------------
# # alphaM_R = (2/Ghost_yMesh)*(rho/rho2)*(-2*Vp - Vs) 
# # betaK_R = (2*Ghost_yMesh)*(1/E_G)*rho*(Vp+Vs)
# # print(f"alphaM_R = {alphaM_R}; betaK_R = {betaK_R}")
# # --------------- Left / Right Rayleigh Damping C = alphaM * M + betaK * K ---------------------------
# region(10001, '-eleRange ', LeftEle, LeftEle+ny-1,'-rayleigh', alphaM_L, betaK_L, betaKinit,betaKcomm) 
# region(10002, '-eleRange ', RightEle, RightEle+ny-1,'-rayleigh',alphaM_R, betaK_R, betaKinit,betaKcomm) 
...
#     timeSeries('Path',xTimeSeriesID+g, '-filePath',f'Ray_S_Sideforce_{ny}rowx/ele{1+g}.txt','-dt', dt) # f'SSideforce_{ny}rowx/ele{1+g}.txt'
...
#     timeSeries('Path',yTimeSeriesID+g, '-filePath',f'Ray_S_Sideforce_{ny}rowy/ele{1+g}.txt','-dt', dt) # f'SSideforce_{ny}rowy/ele{1+g}.txt'
...
timeSeries('Path',702, '-filePath', f'TimeSeries/fs200_{nx}row.txt','-dt', dt) # 2*sinwst *************
...
# recorder('Node', '-file', f'{path1}/Reac{LowerN_Center}.out', '-time', '-node',LowerN_Center,'-dof',1,2,3,'reaction')
# recorder('Node', '-file', f'{path1}/Reac{LowerN_Right}.out', '-time', '-node',LowerN_Right,'-dof',1,2,3,'reaction')
# # =========== Make Integrator Dt Calculate : Newmark(r=0.5, B=0.25) => dt/Tn <= infinite  ===================
# # time_Scale = 1e-4
# Vs_2 = time_Scale*Vs     ;# m/s 

# dcell_2 = yMesh / Vs_2 
# dt_2 = dcell_2/10.0  #6.25e-5
# print(f"Vs_2 = {Vs_2}; dt_2 = {dt_2}")
# # ======= Totla Analysis Integrator Time ============================
# # analysisTime2 = (soilLength/Vs_2)*8
# analysisTime2 = analysisTime/time_Scale #analysisTime*10
# analystep_2 = analysisTime2/(dt_2*2) 
# print(f"Analysis Integrator Time = {analysisTime2} ;Analysis_step = {analystep_2}")


# ------------- left column -------------Stress /Velocity {path2}/{path1}
recorder('Element', '-file', f'Stress/ele1.out', '-time', '-ele',1, 'material ',1,'stresses')
recorder('Element', '-file', f'Stress/ele41.out', '-time', '-ele',41, 'material ',1,'stresses')
recorder('Element', '-file', f'Stress/ele80.out', '-time', '-ele',80, 'material ',1,'stresses')

recorder('Node', '-file', f'Velocity/node82.out', '-time', '-node',82,'-dof',1,2,3,'vel') 
recorder('Node', '-file', f'Velocity/node122.out', '-time', '-node',122,'-dof',1,2,3,'vel')
recorder('Node', '-file', f'Velocity/node162.out', '-time', '-node',162,'-dof',1,2,3,'vel')

# ------------- Right column -------------
recorder('Node', '-file', f'Velocity/node1.out', '-time', '-node',1,'-dof',1,2,3,'vel')
recorder('Node', '-file', f'Velocity/node41.out', '-time', '-node',41,'-dof',1,2,3,'vel')
recorder('Node', '-file', f'Velocity/node81.out', '-time', '-node',81,'-dof',1,2,3,'vel')
