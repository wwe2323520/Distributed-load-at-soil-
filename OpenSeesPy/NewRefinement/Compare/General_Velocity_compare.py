...
# ------------------- File Path Name --------------------
Boundary = 'SideDash'
Boundary1 = 'LK Dashpot Boundary Condition'
...
def Differ_SoilWidthVel(total_time,wave1,Width20_Mid80row,Width10_Mid80row,Width1_Mid80row):
    ...
    plt.plot(total_time,wave1[:,99],label = r'$\mathrm{Analytical}$',color= 'black',linewidth=2.0)
    plt.plot(Width20_Mid80row[:,0],Width20_Mid80row[:,plt_axis2],label ='20m Soil', ls = '--',color= 'darkorange',linewidth=6.0)
    plt.plot(Width10_Mid80row[:,0],Width10_Mid80row[:,plt_axis2],label ='10m Soil', ls = '-.',color= 'limegreen',linewidth=4.0)
    plt.plot(Width1_Mid80row[:,0],Width1_Mid80row[:,plt_axis2],label ='1m Soil', ls = ':',color= 'red',linewidth=3.0)
...
fig3.text(0.050,0.5, r"$\mathrm {Velocity}$  $v_x$  $\mathrm {(m/s)}$", va= 'center', rotation= 'vertical', fontsize=20)
...
plt.subplots_adjust(left=0.125,
                    ...
                    hspace=0.35)
# # fig3.tight_layout()
# ------------ Draw Different SoilWidth with 'Same' row element: Three Quarter Point -----------------------
fig4, (ax11,ax12,ax13,ax14) = plt.subplots(nrows= 4, ncols=1, sharex=True, figsize=(8,8))
fig4.suptitle(f'{Boundary1} \n(Three Quarter node)',x=0.50,y =0.95,fontsize = 20)
fig4.text(0.050,0.5, r"$\mathrm {Velocity}$  $v_x$  $\mathrm {(m/s)}$", va= 'center', rotation= 'vertical', fontsize=20)
