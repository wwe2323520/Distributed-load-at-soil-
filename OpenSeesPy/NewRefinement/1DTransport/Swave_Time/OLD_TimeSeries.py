...
for j in range(Nele): #100 
    ...
    for i in range(len(time_cs)):      
        xii = x0 + dx*i 
        XIn[5+10*j+i,j] = Incoming_wave(ws_20HZ, xii, cs, tin)  #from 0.05m to 9.95m
...
for j in range(Nele):# 100 Nele
    ...
    for i in range(len(time_cs)):      
        xoo = x0 + dx*i 
        XOut[End_disp-10*j+i,End_Ele-j] = Outgoing_wave(ws_20HZ, xoo, cs, tout)  #from 9.95m to 0.05m      
...
for j in range(Nnode): #Nele 101
    ...
    for i in range(len(timeCs_Node)):      
        xii = x0 + dx*i 
        XNodeIn[0+10*j+i,j] = Incoming_wave(ws_20HZ, xii, cs, tNin)  #from 0.05m to 9.95m
        # print(xii)
...
for j in range(Nnode):# Nnode 101
    ...
    for i in range(len(timeCs_Node)):      
        xoo = x0 + dx*i 
        XNodeOut[Endx0-10*j+i,End_Node-j] = Outgoing_wave(ws_20HZ, xoo, cs, tNout)  #from 10.0m to 0.0m   
