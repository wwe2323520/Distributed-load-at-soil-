...
tns_cs = soilLength/cs # wave transport time
...
time_cs = np.arange(0.0, 2*Input_Time + dt_cs, dt_cs)
Nt = len(time_cs)
#----------- Soil Coordinate --------------
x = cs*time_cs #m
...
total_Transport_cs = np.arange(0.0, 4*L+0.1, dx)
...
total_time = np.arange(0.0,0.4001, dt_cs)
...
def Differ_BCVel(total_time, Swave, Tie, LKDash, BeamType1, BeamType2, BeamType3):
    ...
    plt.plot(total_time, Swave[:, Nele-1],label =r'$\mathrm{Analytical}$',color= 'black',linewidth=6.0)
...
fig1.text(0.55,0.68, r"$\mathrm {Swave}$ $(t_{d} = 1/40)$", color = "blue", fontsize=22)
...
# ax15.set_title(r"$w=$ $\mathrm{2m}$",fontsize =23, x=0.15, y=0.70)
...
# fig5.legend(lines, labels, loc = (0.77,0.12) ,prop=font_props)
...
# # fig6.text(0.15,0.80, r"$\mathrm {Swave}$ $(t_{d} = 1/40)$", color = "blue", fontsize=22)
...
# # ax16.set_title(r"$w=$ $\mathrm{20m}$",fontsize =23, x=0.15, y=0.40)
...
# # ax17.set_title(r"$w=$ $\mathrm{10m}$",fontsize =23, x=0.15, y=0.60)
...
# # ax18.set_title(r"$w=$ $\mathrm{2m}$",fontsize =23, x=0.15, y=0.60)
...
# # fig6.legend(lines, labels, loc = (0.77,0.85),prop=font_props)
