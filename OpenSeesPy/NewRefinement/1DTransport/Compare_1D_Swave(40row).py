...
def LK_RelativeError(Peak,LK2, LK10, LK20):
    ...
    plt.text(0.77, 0.73,'LK Dashpot', color='black', fontsize = 25, transform=plt.gca().transAxes)
    ...
    legend = plt.legend(ncol= 1, loc= (0.70, 0.78), prop=font_props) #ncol=2,fontsize=16 frameon=False
    legend.get_frame().set_edgecolor('grey')
    legend.get_frame().set_linewidth(2)  # 設置外框寬度
...
def LK_L2Error(Peak, LK2, LK10, LK20):
    ...
    # plt.xlim(0.0, 0.20)
    # plt.grid(True)
    legend = plt.legend(ncol= 1, loc= (0.70, 0.02), prop=font_props) #ncol=2,fontsize=16 frameon=False
    legend.get_frame().set_edgecolor('grey')
    legend.get_frame().set_linewidth(2)  # 設置外框寬度
