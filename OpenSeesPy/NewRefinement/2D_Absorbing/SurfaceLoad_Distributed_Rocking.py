...
Choose_Wave = f"NoChange_Dw/Distributed_Rocking" # Pwave ; Vertical / Horizon / Rocking  NoChange_Dw/Distributed_Rocking
# HZ = 40
...
for i in range(len(Width)):
    ...
    for j in range(len(Force_HZ)):
        ...
        for h in range(len(Y_MeshNumber)):
            ...
            soilLength = 10 #m
            # soilwidth = 2.0
            # ny = 80 # 80, 40, 20, 10 ; SurfaceLoad: 40, 20, 10
            ...
            # ================= Build Boundary File =====================
            Boundary = f"{Choose_Wave}/W_{int(soilwidth)}m/HZ_{HZ}/LKDash_Surface_{ny}row"
            ...
            # # # ----------- Point Rocking Load ----------------------
            # load(Found_Left, 0, 1.0*1e5)
            # load(Found_Right, 0, -1.0*1e5)
