...
for j in range(Nele):# 100
    ...
    for i in range(len(time)):      
        xoo = x0 + dx*i 
        XOut[End_disp-10*j+i,End_Ele-j] = Outcoming_wave(xoo,tout)  #from 9.95m to 0.05m      
...
for j in range(Nnode):# Nnode 101
    ...
    for i in range(len(time)):      
        xoo = x0 + dx*i 
        XNodeOut[Endx0-10*j+i,End_Node-j] = Outcoming_wave(xoo,tNout)  #from 10.0m to 0.0m   

total_time = np.arange(0.0,0.4001,dt)
Swave = np.zeros((len(total_time),Nele))

# ===== New BC Sideforce on Left and Right ==============
SNodeforce_x = np.zeros((len(total_time),Nnode)) # 11
...
for m in range(Nnode): #Nnode 101
    t1 = 10*m
    # print(t1)
...
# 建立資料夾
# ---------- Pwave ---------------
S_folder_name_x = "S_Nodeforce_20rowx"
...
# plt.plot(total_time,Swave[:,10],label ='Node 5', marker='x', markevery=100)
# plt.plot(total_time,Swave[:,19],label ='Node 10', marker='d', markevery=100)

# ----- Swave eta_p*(Vx) -------------
plt.plot(total_time,SNodeforce_x[:,0],label ='Node 1', marker='o', markevery=100)
plt.plot(total_time,SNodeforce_x[:,10],label ='Node 10', marker='x', markevery=100)
plt.plot(total_time,SNodeforce_x[:,20],label ='Node 21', marker='d', markevery=100)

# ----- Swave sigma_xy -------------
# plt.plot(total_time,SSideforce_y[:,0],label ='Element 1', marker='o', markevery=100)
# plt.plot(total_time,SSideforce_y[:,10],label ='Element 5', marker='x', markevery=100)
# plt.plot(total_time,SSideforce_y[:,19],label ='Element 10', marker='d', markevery=100)
...
# plt.plot(total_Transport,XNodeIn[:,10],label ='Node 5',marker='d', markevery=100)
# plt.plot(total_Transport,XNodeIn[:,20],label ='Node 11',marker='x', markevery=100)
# plt.legend(loc='upper right',fontsize=18)
# plt.xlim(0,30.0)
# plt.xticks(fontsize = 15)
# plt.yticks(fontsize = 15)
...
# plt.plot(total_Transport,XNodeOut[:,10],label ='Node 10',marker='d', markevery=100)
# plt.plot(total_Transport,XNodeOut[:,20],label ='Node 20',marker='x', markevery=100)
# plt.xlim(0,30.0)
