...
for l in range(ny+1):
# ========= Left Side =============
# --------- Normal dashpot (node 415,416~ 495,496)-> for S wave------------
    node(415+2*l, 0.0, yMesh*l)
    node(416+2*l, 0.0, yMesh*l)
# ---------- dashpot dir: Vs -> x dir ---------------------     
    fix(415+2*l, 0, 1, 1)      # x dir dashpot　
    fix(416+2*l, 1, 1, 1)      # fixed end to let soil fix
 
# --------- Traction dashpot (node 497,498~ 577,578)-> for P wave------------
    node(497+2*l, 0.0, yMesh*l)
    node(498+2*l, 0.0, yMesh*l)
# ---------- dashpot dir: Vp -> y dir ---------------------     
    fix(497+2*l, 1, 0, 1)      # y dir dashpot　
    fix(498+2*l, 1, 1, 1)      # fixed end to let soil fix

# ========= Right Side =============
# --------- Normal dashpot (node 579,580~ 659,660)-> for S wave------------
    node(579+2*l, 1.0, yMesh*l)
    node(580+2*l, 1.0, yMesh*l)
# ---------- dashpot dir: Vs -> x dir ---------------------     
    fix(579+2*l, 0, 1, 1)      # x dir dashpot　
    fix(580+2*l, 1, 1, 1)      # fixed end to let soil fix

# --------- Traction dashpot (node 661,662~ 741,742)-> for P wave------------
    node(661+2*l, 1.0, yMesh*l)
    node(662+2*l, 1.0, yMesh*l)
# ---------- dashpot dir: Vp -> y dir ---------------------     
    fix(661+2*l, 1, 0, 1)      # y dir dashpot　
    fix(662+2*l, 1, 1, 1)      # fixed end to let soil fix

# ------ connect dashpot with Soil side layer :Vs with x dir / Vp with y-dir --------------
for l in range(ny+1):
# ========= Left Side =============
# --------------Normal dashpot: for S wave------------------
    equalDOF(1+9*l, 415+2*l, 1)  # x dir
# --------------Traction dashpot: for P wave------------------
    equalDOF(1+9*l, 497+2*l, 2)  # y dir

# ========= Right Side =============
# --------------Normal dashpot: for S wave------------------
    equalDOF(9+9*l, 579+2*l, 1)  # x dir
# --------------Traction dashpot: for P wave------------------
    equalDOF(9+9*l, 661+2*l, 2)  # y dir
...
#----------- Left side Traction Dashpot: (ele 388~428) ---------- -> Sms
    element('zeroLength',388+w, 498+2*w, 497+2*w, '-mat',4007,'-dir',ydir)  # center node：P wave
 
#----------- Right side Normal Dashpot:(ele 429 ~ 469) ---------- -> Smp
    element('zeroLength',429+w, 580+2*w, 579+2*w, '-mat',4006,'-dir',xdir)  # center node : S wave
#----------- Right side Traction Dashpot: (ele 470 ~ 510) ----------  -> Sms
    element('zeroLength',470+w, 662+2*w, 661+2*w, '-mat',4007,'-dir',ydir)  # center node：P wave

print("Finished creating Side dashpot material and element...")

# ==================== Side Beam node (743~783 / 784~824) ====================
for i in range(ny+1):
# ----- Left Side: 403~423 -----------------
    node(743+i, 0.0, yMesh*i)
    fix(743+i,0,0,1)
# ----- Right Side: 784~824 -----------------
    node(784+i,1.0,yMesh*i)
    fix(784+i,0,0,1)

# ------------  Beam Element: 271 ~ 290 / 291 ~ 310 ------------------
for j in range(ny):
# ----- Left Side Beam:511 ~ 550 -----------------
    element('elasticBeamColumn', 511+j, 743+j, 744+j, A,E1,Iz, 1, '-release', 3)
# ----- Right Side Beam:551 ~ 590 -----------------
    element('elasticBeamColumn', 551+j, 784+j, 785+j, A,E1,Iz, 1, '-release', 3)

# --------- Side Beam and Soil BC -----------------
for j in range(ny+1):
    equalDOF(1+9*j,743+j,1,2)
    equalDOF(9+9*j,784+j,1,2)
...
# ---------- Distributed at Left Side Beam ----------------------
    eleLoad('-ele',511+g, '-type', '-beamUniform',-20,0)  # for local axes Wy -
# ---------- Distributed at Right Side Beam ----------------------
    eleLoad('-ele',551+g, '-type', '-beamUniform',-20,0)   # for local axes Wy +
...
# ---------- Distributed at Left Side Beam ----------------------
    eleLoad('-ele',511+g, '-type', '-beamUniform',0,+20,0)  # for local axes Wx +
# ---------- Distributed at Right Side Beam ----------------------
    eleLoad('-ele',551+g, '-type', '-beamUniform',0,-20,0)   # for local axes Wx -
