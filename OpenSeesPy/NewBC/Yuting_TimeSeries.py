...
# ------- For P wave ------------------------------------
            # print(j)
        if time[j] <= 0.1 and i != 99: 
            # Sideforce_y[cpid+j,i] = -eta_s*PVel_FF[j]*A
            Sideforce_x[cpid+j,i] = (nu/(1-nu))*np.sin(wp*(time[j]-0.0))   
        elif (time[j] > 0.1 and time[j] <= 0.2 and i!= 99): 
                # Sideforce_y[cpid+j,99-i] = -eta_s*PVel_FF[j-step]*A #-
            Sideforce_x[cpid+j, i] = -(nu/(1-nu))*np.sin(wp*(time[j]-0.1)) #-     
            
        if time[j] <= 0.099 and i == 99:
            Sideforce_x[cpid+j,i] = (nu/(1-nu))*np.sin(wp*(time[j]-0.0))
        elif (time[j] > 0.099 and time[j] <= 0.199 and i == 99):
            Sideforce_x[cpid+j, i] = -(nu/(1-nu))*np.sin(wp*(time[j]-0.099)) #-  
...
# plt.plot(time,Sideforce_x[:,99],label = force_id[0],marker='o', markevery=100)
plt.plot(time,Sideforce_x[:,0],label = force_id[0],marker='o', markevery=100)
plt.plot(time,Sideforce_x[:,50],label = force_id[50],marker='x', markevery=100)
plt.plot(time,Sideforce_x[:,99],label = force_id[99],marker='d', markevery=100)
